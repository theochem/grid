...
class Symmetry(object):
    ...
    def __init__(self, name, generators, fracs, numbers, cell, labels=None):
        ...
        if labels is None:
            ...
            if len(labels) != numbers.shape[0]:
                raise TypeError(
                    'The length of the labels list is not consistent with the number of atoms in the primitive unit.')
    ...
    def identify(self, coordinates, cell, threshold=0.1):
        ...
        for k in range(natom):
            ...
            if not match:
                raise SymmetryError(
                    'Could not find atom in primitive unit corresponding to atom %i' % k)
