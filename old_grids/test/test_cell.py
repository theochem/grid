...
def test_cell_quartz():
    cell = Cell(np.array(
        [[0.0, 0.0, 5.405222], [0.0, 4.913416, 0.0], [-4.255154, 2.456708, 0.0]]) * angstrom)
...
def test_ranges_rcut_3d():
    counter = 0
    while True:
        cell, origin, center, rcut, ranges_begin, ranges_end, ranges_low, ranges_high = setup_ranges_rcut(
            3)
        ...
        for i0 in range(ranges_low[0], ranges_high[0]):
            for i1 in range(ranges_low[1], ranges_high[1]):
                for i2 in range(ranges_low[2], ranges_high[2]):
                    if (ranges_begin[0] <= i0 < ranges_end[0] and
                            ranges_begin[1] <= i1 < ranges_end[1] and
                            ranges_begin[2] <= i2 < ranges_end[2]):
                        continue
                    tmp = origin.copy()
...
def test_ranges_rcut_2d():
    ...
    while True:
        ...
        for i0 in range(ranges_low[0], ranges_high[0]):
            for i1 in range(ranges_low[1], ranges_high[1]):
                if (ranges_begin[0] <= i0 < ranges_end[0] and
                        ranges_begin[1] <= i1 < ranges_end[1]):
                    continue
                tmp = origin.copy()
...
def test_ranges_rcut_1d():
    ...
    while True:
        ...
        # test if distances to points outside the ranges are always outside rcut
        for i0 in range(ranges_low[0], ranges_high[0]):
            if ranges_begin[0] <= i0 < ranges_end[0]:
                continue
            tmp = origin.copy()
