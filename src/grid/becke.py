...
def compute_becke_alpha(radii, cutoff=0.45):
    ...
    -------
    np.ndarray(N, N)
        Value of alpha for each pair of atoms
...
def compute_becke_switch_func(x, order=3):
...
def generate_becke_weights(
    ...
    -------
    np.ndarray(M, )
        Becke weights evaluated on each grid point
    ...
    del p_p_n
    alpha = compute_becke_alpha(radii)
    v_pp = mu_n_p_p + alpha * (1 - mu_n_p_p ** 2)
    del mu_n_p_p
    s_ab = 0.5 * (1 - compute_becke_switch_func(v_pp, order=order))
    del v_pp
    # convert nan to 1
