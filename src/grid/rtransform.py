...
class BeckeTF(BaseTransform):
    ...
    def transform(self, r, trim_inf=True):
        ...
        """
        rf_array = self._R * (1 + r) / (1 - r) + self._r0
        if trim_inf:
...
class LinearTF(BaseTransform):
    ...
    def transform(self, r):
        ...
        """
        return (self._rmax - self._rmin) / 2 * (1 + r) + self._rmin
    ...
    def deriv(self, r):
        ...
        if isinstance(r, Number):
            ...
            return np.ones(r.size) * (self._rmax - self._rmin) / 2
    ...
    def deriv2(self, r):
        ...
        """
        return np.array(0) if isinstance(r, Number) else np.zeros(r.size)
...
class IdentityRTransform(BaseTransform):
    ...
    def deriv(self, r: np.ndarray):
        ...
        return 1 if isinstance(r, Number) else np.ones(r.size)
...
class PowerRTransform(BaseTransform):
    ...
    def transform(self, r: np.ndarray):
        ...
        return self._rmin * np.power(r + 1, power)

    def deriv(self, r: np.ndarray):
        """Compute first derivative of power transform."""
        power = (np.log(self._rmax) - np.log(self._rmin)) / np.log(r.size)
        return power * self._rmin * np.power(r + 1, power - 1)

    def deriv2(self, r: np.ndarray):
        """Compute second derivative of power transform."""
        power = (np.log(self._rmax) - np.log(self._rmin)) / np.log(r.size)
        return power * (power - 1) * self._rmin * np.power(r + 1, power - 2)

    def deriv3(self, r: np.ndarray):
        """Compute third derivative of power transform."""
        power = (np.log(self._rmax) - np.log(self._rmin)) / np.log(r.size)
        return (
            power * (power - 1) * (power - 2) * self._rmin * np.power(r + 1, power - 3)
        )
