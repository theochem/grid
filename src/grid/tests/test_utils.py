...
class TestUtils(TestCase):
    ...
    def test_get_atomic_radii(self):
        ...
        Bragg_Slater = np.array([
            0.25, np.nan, 1.45, 1.05, 0.85, 0.7, 0.65, 0.6, 0.5, np.nan, 1.8,
            1.5, 1.25, 1.1, 1., 1., 1., np.nan, 2.2, 1.8, 1.6, 1.4, 1.35, 1.4,
            1.4, 1.4, 1.35, 1.35, 1.35, 1.35, 1.3, 1.25, 1.15, 1.15, 1.15,
        ...
        Cambridge = np.array([
            ...
            1.41, 1.21, 1.11, 1.07, 1.05, 1.02, 1.06, 2.03, 1.76, 1.7, 1.6,
        ...
        # fmt: on
        all_index = np.arange(86)
        bragg = get_cov_radii(all_index, type="bragg")
        assert_allclose(bragg, Bragg_Slater * 1.8897261339213)
        bragg = get_cov_radii(all_index, type="cambridge")
        assert_allclose(bragg, Cambridge * 1.8897261339213)
