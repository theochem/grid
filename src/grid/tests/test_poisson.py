...
def test_interpolation_of_laplacian_with_unit_charge_distribution():
    ...
    assert_allclose(-4.0 * np.pi * charge_distribution(atgrid.points), true,
                    atol=1e-4, rtol=1e-7)
...
def test_poisson_bvp_on_unit_charge_distribution(oned, tf, remove_large_pts):
    ...
    potential = solve_poisson_bvp(atgrid, charge_distribution(atgrid.points), InverseRTransform(tf),
...
@pytest.mark.parametrize(
    ...
    [
        ...
        [spherical_harmonic, InverseRTransform(BeckeRTransform(1e-2, 1.5, trim_inf=True))],
...
def test_poisson_bvp_gives_the_correct_laplacian(func, tf):
    ...
    assert_allclose(laplace_pot(random_pts), -func(random_pts) * 4.0 * np.pi,
                    atol=1e-2)
...
def test_poisson_on_unit_charge_distribution_ivp(oned, tf):
    ...
    potential = solve_poisson_ivp(atgrid, charge_distribution(atgrid.points), InverseRTransform(tf),
