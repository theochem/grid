...
class AtomGrid(Grid):
    ...
    def solve_poisson(self, value_array, x_range, boundary, tfm=None):
        """Solve given poisson equation with boundary condition

        Parameters
        ----------
        value_array : np.ndarray(N,)
            property value array at each grid point of an atom
        x_range : np.ndarray(M,)
            initial mesh of x values for solving PDE
        boundary : float
            boundary condition value when x -> inf
        tfm : RTransform
            reverse transformation object for radial grid
        ...
        for index, spl in enumerate(splines):
            res = Poisson.solve_poisson_bv(
                spl, x_range, boundary, ml_pairs[index], tfm=tfm
            )
            # print(index, res(1.))
            result.append(res)
        return result
    ...
    @staticmethod
    def _generate_ml_for_sph(n_sphs):
        """Generate m, l pairs for generated spherical harmonics

        Parameters
        ----------
        n_sphs : int
            The total number of spherical harmonics generated by
            `_generate_real_sph_harm` method
        ...
        ------
        ValueError
            n_sphs needs to be n^2
        """
        if not np.sqrt(n_sphs).is_integer() or n_sphs < 1:
            raise ValueError(f"the total number is not valid, got {n_sphs}")
        result = []
        nshells = int(np.sqrt(n_sphs))
        for m in range(nshells):
            # add 0
            result.append((m, 0))
            # add positive l
            result.extend(zip([m] * m, np.arange(1, m + 1)))
            # add negative l
            result.extend(zip([m] * m, np.arange(-m, 0)))
