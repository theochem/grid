...
__all__ = ["CubicProTransform", "PromolMolecule"]
...
class PromolMolecule:
    """
    Simplified interface for creating CubicProTransform grids using BFit data.

    Parameters
    ----------
    symbols : list of str
        Element symbols for the molecular system.
    coords : np.ndarray(M, 3)
        Atomic coordinates in atomic units.
    expected_electrons : int
        Expected number of electrons in the system.
    bfit_data_file : str or Path
        Path to the BFit data file containing promolecular parameters.

    Examples
    --------
    >>> symbols = ['h', 'h']
    >>> coords = np.array([[0., 0., -0.5], [0., 0., 0.5]])
    >>> mol = PromolMolecule(symbols, coords, 2, 'data/bfit_data.npz')
    >>> transform_grid = mol.create_transform_grid(grid_size=20)
    """

    def __init__(self, symbols, coords, expected_electrons, bfit_data_file):
        self.symbols = symbols
        self.coords = np.array(coords)
        self.expected_electrons = expected_electrons
        self.bfit_data_file = bfit_data_file
        self._bfit_processor = None
        self._normalized_promol = None

    def _initialize_bfit_processor(self):
        """Initialize BFit processor if not already done."""
        if self._bfit_processor is None:
            from grid.utils_promol import BFitDataProcessor
            self._bfit_processor = BFitDataProcessor(self.bfit_data_file)
            if not self._bfit_processor.load_bfit_data():
                raise RuntimeError(f"Failed to load BFit data from {self.bfit_data_file}")
            self._bfit_processor.parse_element_data()

    def _create_normalized_promol_params(self):
        """Create normalized promolecular parameters."""
        if self._normalized_promol is None:
            self._initialize_bfit_processor()

            # Check element availability
            missing = [s for s in self.symbols if
                       s.lower() not in self._bfit_processor.element_data]
            if missing:
                raise ValueError(f"Missing elements in BFit data: {missing}")

            # Create dependencies dict for BFitDataProcessor
            deps = {
            ...
            promol_params = self._bfit_processor.create_promol_params(
                self.symbols, self.coords, deps
            )

            # Normalize to match expected electron count
            norm_factor = self.expected_electrons / promol_params.integrate_all()

            self._normalized_promol = _PromolParams(
        ...
        return self._normalized_promol

    def create_transform_grid(self, grid_size=20, oned_grid_type="GaussChebyshev"):
        """
        Create a CubicProTransform grid for the molecular system.

        Parameters
        ----------
        grid_size : int, optional
            Number of points in each dimension. Default is 20.
        oned_grid_type : str, optional
            Type of one-dimensional grid to use. Default is "GaussChebyshev".

        Returns
        -------
        CubicProTransform
            The constructed transform grid object.
        """
        # Get normalized promolecular parameters
        normalized_promol = self._create_normalized_promol_params()

        # Create one-dimensional grids
        if oned_grid_type == "GaussChebyshev":
            from grid.onedgrid import GaussChebyshev
            oned_grid = GaussChebyshev(grid_size)
        else:
            raise ValueError(f"Unsupported oned_grid_type: {oned_grid_type}")

        # Create and return CubicProTransform
        return CubicProTransform(
