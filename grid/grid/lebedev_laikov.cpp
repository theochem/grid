...
// and J. Chem. Phys. 122, 244510 (2005).
//
// The implementation in Orientation is close to the original one created by
// Laikov and Lebedev, with just a few modifications to use it in a C++ program.
// These modificatons were done by Marina Carravetta, who granted permission to
...
/* lebedev_laikov_oh
   ...
   Fill in datapoints in the grid according to some symmetry parameter.
   ...
   **Arguments:**
   ...
   a,b
      Determine the rotation of the grid. These are only used by the symmetry
      cases that support them.
...
void lebedev_laikov_sphere(int npoint, double* points, double* weights)
{
  ...
  switch (npoint) {
    ...
    case 5810:
      ...
      break;
    default:
      throw std::domain_error("Grid size not supported.");
  }
  if (n != npoint) {
    throw std::runtime_error("Internal inconsistency in lebedev_laikov_sphere.");
  }
}
#undef A
#undef B
