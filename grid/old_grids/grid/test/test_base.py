...
def test_eval_spline_grid_2d_random():
    ...
    for i in range(10):
        ...
        rvecs = cell.rvecs
        points = np.dot(np.random.normal(-2, 3, (npoint, 2)), rvecs) + np.random.normal(-3, 3,
                                                                                        (npoint, 3))
        g = IntGrid(points, np.random.normal(0, 1.0, npoint))
...
def test_density_decomposition_n2():
    ...
    mol = IOData.from_file(pkg_resources.resource_filename('grid.test.data','n2_hfs_sto3g.fchk'))
    ...
    rho = obasis.compute_grid_density_dm(dm_full, atgrid.points)
