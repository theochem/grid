...
class RTransform:
    ...
    def from_string(cls, s: str):
        ...
        if clsname == 'IdentityRTransform':
            if len(args) != 1:
                raise ValueError(
                    'The IdentityRTransform needs one argument, got %i.' %
                    len(words))
            npoint = int(args[0])
            ...
            if len(args) != 3:
                raise ValueError(
                    'The LinearRTransform needs three arguments, got %i.' %
                    len(words))
            rmin = float(args[0])
            rmax = float(args[1])
            npoint = int(args[2])
            ...
            if len(args) != 3:
                raise ValueError(
                    'The ExpRTransform needs three arguments, got %i.' %
                    len(words))
            rmin = float(args[0])
            rmax = float(args[1])
            npoint = int(args[2])
            ...
            if len(args) != 3:
                raise ValueError(
                    'The PowerRTransform needs three arguments, got %i.' %
                    len(words))
            rmin = float(args[0])
            rmax = float(args[1])
            npoint = int(args[2])
...
class PowerRTransform(RTransform):
    ...
    def _deriv2(self, t: float):
        return self._power * (self._power - 1) * self._rmin * np.power(
            t + 1, self._power - 2)

    def _deriv3(self, t: float):
        return self._power * (self._power - 1) * (
            self._power - 2) * self._rmin * np.power(t + 1, self._power - 3)

    def _inv(self, r: float):
        return np.power(r / self._rmin, 1. / self._power) - 1
...
class HyperbolicRTransform(RTransform):
    ...
    def _deriv2(self, t: float):
        x = 1. / (1 - self._b * t)
        return 2. * self._a * self._b * x**3

    def _deriv3(self, t: float):
        x = 1. / (1 - self._b * t)
        return 6. * self._a * self._b * self._b * x**4
