...
from grid.grid.lebedev import lebedev_laikov_sphere, lebedev_laikov_npoints
...
class AtomicGrid(IntGrid):
    ...
    def __init__(self, number, pseudo_number, center, agspec='medium', random_rotate=True,
                 points=None):
        """
           **Arguments:**
...
def _normalize_nlls(nlls, size):
    ...
    if hasattr(nlls, '__iter__'):
        if len(nlls) == 1:
            ...
            if len(nlls) != size:
                raise ValueError(
                    'The size of the radial grid must match the number of elements in nlls')
    else:
...
class AtomicGridSpec(object):
           ...
           **Optional argument:**
           ...
           definition
                ...
                2. It can be any of 'coarse', 'medium', 'fine', 'veryfine',
                   ...
                   one-to-one mapping with the files in ${OLDGRIDSDATA}/grids.
                3. It can be the name of a file in ${OLDGRIDSDATA}/grids (without
                   the extension ``.txt``
    ...
    def get(self, number, pseudo_number):
        for pn, rgrid, nlls in self.members.get(number, []):
            if pn >= pseudo_number:
                return rgrid, nlls
        raise ValueError(
            'The atomic grid specification "%s" does not support element %i with effective core %i' % (
                self.name, number, pseudo_number))
    ...
    def _init_members_from_list(self, members):
        ...
        for l in list(self.members.values()):
            l.sort()
